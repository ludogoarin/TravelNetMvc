...
namespace Caval_go.Content.ckeditor
{
    public partial class ImageBrowser : System.Web.UI.Page
    {
        ...
        {
            float ratio = CkUtil.Parse<float>(ImageAspectRatio.Value);
            int width = CkUtil.Parse<int>(ResizeWidth.Text);
        ...
        {
            float ratio = CkUtil.Parse<float>(ImageAspectRatio.Value);
            int height = CkUtil.Parse<int>(ResizeHeight.Text);
        ...
        protected void Upload(object sender, EventArgs e)
        {
            if (IsImage(UploadedImageFile.FileName))
            {
                ...
                var data = UploadedImageFile.FileBytes.ToImage().Resize(960, null, null).ToByteArray();
